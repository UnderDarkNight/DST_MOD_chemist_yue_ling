...
local assets =
{
    Asset("ANIM", "anim/firenettles_seeds.zip"),
...
local function common(anim, cookable, oceanfishing_lure)
    ...
    inst.AnimState:SetBank("firenettles_seeds")
    inst.AnimState:SetBuild("firenettles_seeds")
    ...
    inst.components.inventoryitem.imagename = "firenettles_seeds"
    inst.components.inventoryitem.atlasname = "images/inventoryimages/firenettles_seeds.xml"
...
local function OnDeploy(inst, pt, deployer) --, rot)
    ...
    local plant = SpawnPrefab("weed_firenettle")
    plant.Transform:SetPosition(pt.x, 0, pt.z)
    TheWorld.Map:CollapseSoilAtPoint(pt.x, 0, pt.z)
    -- inst:Remove()
...
return Prefab("firenettles_seeds", raw, assets, prefabs),
    MakePlacer("firenettles_seeds_placer", "farm_soil", "farm_soil", "till_idle", nil, nil, nil, nil, nil, nil, seed_placer_postinit)
